...
function findAllPSiblings(where) {
    ...
    for (let i = 0; i < childrens.length; i++) {
        if (childrens[i].tagName === 'P') {
            arr.push(childrens[i].previousElementSibling);
        }
    }
...
function deleteTextNodes(where) {
    let childrenNodes = where.childNodes;
...
function deleteTextNodesRecursive(where) {
    ...
    for (let i = 0; i < childrenNodes.length; i++) {
        if (childrenNodes[i].nodeType === 3) {
            where.removeChild(childrenNodes[i]);

            i--;
        } else if (childrenNodes[i].nodeType === 1) {
            deleteTextNodesRecursive(childrenNodes[i]);
        }
    }
}
